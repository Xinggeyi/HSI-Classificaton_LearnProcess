...
from keras.layers import Dense, CuDNNGRU, CuDNNLSTM, Flatten, SimpleRNN, LSTM, GRU
...
def set_params(args):
    args.batch_size = 100; args.epochs = 50    ## 要修改
    return args

def get_model_compiled(feat_size, seq_len, num_class, type_func):
    if type_func == "RNN": func = SimpleRNN
    # elif type_func == "GRU": func = CuDNNGRU    # 只能在GPU运行
    # elif type_func == "LSTM": func = CuDNNLSTM  # 只能在GPU运行
    elif type_func == "GRU": func = GRU
    elif type_func == "LSTM": func = LSTM
...
def main():
    ...
    parser.add_argument('--epochs', default=10, type=int, help='Number of full training cycle on the training set')
    #########################################
    ...
    for pos in range(args.repeat):
        ...
        clf = get_model_compiled(n_bands, sequences, num_class, args.type_recurrent)
